...
const orderCartOptions = createSlice({
  ...
  reducers: {
    setProductsData: (state, action: PayloadAction<any>) => {
      ...
      const updatedProductsStock = productsStock.map((stockProduct: any) => {
        ...
        if (cartProduct) {
          return {
            ...stockProduct,
            stock_quantity: stockProduct.stock_quantity - cartProduct.quantity,
          };
        }
        return stockProduct;
      });
    ...
    setPartsData: (state, action: PayloadAction<any>) => {
      ...
      const updatedPartsStock = partsStock.map((stockPart: any) => {
