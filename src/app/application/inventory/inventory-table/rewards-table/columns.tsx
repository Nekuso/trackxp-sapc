...
import { allRewardsDisplay } from "@/types";
...
export const initialState = (branches: any) => {
  const columns: ColumnDef<allRewardsDisplay>[] = [
    {
      id: "name",
      accessorKey: "name",
      header: ({ column }) => {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <p className="max-w-[190px] 2xl:max-w-[220px] truncate font-bold flex place-items-center gap-1">
            <MdStars className="h-full w-5 text-center" />
            {row.original.points_required}
          </p>
        );
      },
    },
      ...
      cell: ({ row }) => {
        ...
        return (
          <Link
            ...
            href={`/application/inventory/reward/${id}`}
          >
